...
const PanController = React.createClass({
  ...
  handleResponderMove(anim, delta, min, max, overshoot) {
    ...
    // if difference is more then take action
    let difference = Math.abs(anim._value) - Math.abs(val)
    
    if(difference > 200)
    {
      anim._offset = -1*difference;
      val = anim._offset + delta;
    }
