...
import axios from 'axios';
 ...
 class  InviteFromPhone extends Component{
  ...
  createMessage(name, location, link){
      return name + " just invited you to a party at " + location + ". Download socialvite to join " + link
  }
  ...
  sendInvitation(friends,username){
    ...
    const {name, location } = this.props.data.info

    return axios.post(AUTH_SERVER, {
            'phone': friendsList,
            'name' : username,
            'msg'  : this.createMessage(username, location.address, "http://apple.co/2voHK7Z")
        });
}
  ...
  getCollection(contacts){
    ...
    db.filterCollection(collection)
        .then(({registered, notregistered}) => {
          sorted_notregistered = this.sortArrayAsc(notregistered, 'fullName')
            this.setState({registered,notregistered : sorted_notregistered, loading: false})
        })
  ...
  onInviteSend(uid, registered, value){
    const {user} = this.props;
      ...
      // need to save on events data events.inviteUsers
...
export default connect (mapStateToDispatch,bindActions) (InviteFromPhone);
